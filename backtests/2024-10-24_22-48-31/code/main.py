...
class TurtleTradingStrategy(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        for symbol in self.symbols:
            ...
            self.entry_channels[symbol] = self.DONCHIAN(symbol, self.ENTRY_CHANNEL)
            self.exit_channels[symbol] = self.DONCHIAN(symbol, self.EXIT_CHANNEL)
            self.atrs[symbol] = self.ATR(symbol, self.ATR_PERIOD, MovingAverageType.Simple)
    ...
    def OnData(self, slice):
        ...
        for symbol in self.symbols:
            self.Log(f"Checking symbol: {symbol}")
