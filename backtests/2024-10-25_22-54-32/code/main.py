...
class TurtleTradingStrategy(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.SetEndDate(2024, 10, 7)  # Set a specific end date
        self.SetCash(100000)
        ...
        self.SetWarmUp(timedelta(days=self.ENTRY_CHANNEL * 2))  # Increase warm-up period
        self.Log(f"Warm-up period set to {self.ENTRY_CHANNEL * 2} days")
    ...
    def OnData(self, slice):
        if self.IsWarmingUp:
            self.Log("Algorithm is still warming up")
            return
        ...
        for symbol in self.symbols:
            entry_ready = self.entry_channels[symbol].IsReady
            exit_ready = self.exit_channels[symbol].IsReady
            atr_ready = self.atrs[symbol].IsReady
            
            self.Log(f"Indicator status for {symbol}:")
            self.Log(f"  Entry Channel: Ready={entry_ready}, Samples={self.entry_channels[symbol].Samples}, Upper={self.entry_channels[symbol].Upper.Current.Value}, Lower={self.entry_channels[symbol].Lower.Current.Value}")
            self.Log(f"  Exit Channel: Ready={exit_ready}, Samples={self.exit_channels[symbol].Samples}, Upper={self.exit_channels[symbol].Upper.Current.Value}, Lower={self.exit_channels[symbol].Lower.Current.Value}")
            self.Log(f"  ATR: Ready={atr_ready}, Value={self.atrs[symbol].Current.Value}")
            
            if not (entry_ready and exit_ready and atr_ready):
                all_indicators_ready = False

        if not all_indicators_ready:
            self.Log("Not all indicators are ready")
            return
